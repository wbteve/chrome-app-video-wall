...
function snowstack_update(newIndex, newmagnifymode)
{
	...
	{
		setcellclass(cell, "cell selected target");
	}


//console.log( cell.info.title);
...
function parseImgUrl(link){
 link = link.replace("{tag}", "default");
 link = link.replace("{width}", "240");	
 link = link.replace("{height}", "180");	
...
function snowstack_addimage(info)
{
	...
	//console.log("snowstack_addimage", info);
	var cell = {};
	var n = cells.length;
	cells.push(cell);
	...
	//{
		//console.log(info.video.url);
		var imageurl = parseImgUrl(info.photo.imageUrl);
	//	console.log(info.video);
		if(info.video == null){
			videoLink = "";
		}else
		{
			videoLink = info.video.url ;
		}
		//console.log("info",info);
		info = { "thumb": imageurl, "zoom": imageurl, "title": info.headline, "video": videoLink, "date" : info.date}; //
	//}
	...
	//console.log("inf ", cell.info);

	cell.div = make_celldiv();

	cell.divimage = vfx.elem("img", { "class": "media" });
	
	cell.divvideo = vfx.elem("video", { "class": "video", "autoplay" : false, "currentSrc" : cell.info.video });
	...
	{ 
		...
		if (cell.info.link)
		{
			cell.div.appendChild(vfx.elem("a", { "class": "mover view", "href": cell.info.link, "target": "_blank" }, cell.divimage)); //change here for lightbox ja
		}
		else
		{
		...
		cell.div.appendChild(vfx.elem("div", { "class": "mover view" }, cell.divvideo));
...
function snowstack_sort(sortkey)
{
	...
	if (sortkey == 'date') 
	{
		...
		{
			//console.log(a.info);  //1000*
			
			var adate = new Date(1000*a.info.date);
			var bdate = new Date(1000*b.info.date);
